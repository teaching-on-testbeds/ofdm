...
def demodulate(p, r, s0, eps, cfo_correct=True, pilot_track=True):
    ...
    # Which subcarriers were on: look at the received power after CFO
    # correction (always, so that the answer doesn't change when you turn
    # the correction off). A subcarrier that is on (or off) in almost every
    # symbol is taken as on (or off) in all of them.
    Yc = np.fft.fft(rc[idx], axis=1)
    Hc = np.where(used, Yc[1] * ce, 1)
    on = (np.abs(Yc[2:] / Hc) ** 2 > 0.25) & used[None, :]
    frac = on.mean(axis=0)
    on[:, frac > 0.9] = True
    on[:, frac < 0.1] = False
    on &= used[None, :]
